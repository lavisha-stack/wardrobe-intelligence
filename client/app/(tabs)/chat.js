...
// AI Assist should always prefer the processed transparent cutout.
// Original photos are only a fallback for older wardrobe items that have
// not yet received a normalized image.
function itemImageSource(item) {
  return { uri: item.normalized_image_url || item.image_url };
}
    ...
    <View style={[styles.stage, expanded ? styles.stageExpanded : styles.stageCompact]}>
      <View style={styles.figureArea}>
        {groups.full ? (
          ...
          <>
            {groups.top && <Image source={itemImageSource(groups.top)} style={styles.topImage} resizeMode="contain" />}
            {groups.bottom && <Image source={itemImageSource(groups.bottom)} style={styles.bottomImage} resizeMode="contain" />}
        ...
        {messages.length === 0 ? (
          <View style={styles.emptyConversation}>
            ...
            <Text style={styles.emptyText}>
              Tell me about your day, your plans, the weather, or even just your vibe.
            </Text>
          </View>
        ) : (
          <>
            {messages.map((message) => (
              <View key={message.id} style={styles.messageBlock}>
                ...
                {message.type === "outfits" && (
                  <OutfitResults outfits={message.outfits} onOpenOutfit={handleOpenOutfit} />
                )}
              </View>
            ))}
              ...
              <Pressable onPress={() => {}}>
                <View style={styles.modalHeader}>
                  <View style={styles.modalTitleBlock}>
                    ...
                    {!!expandedOutfit.title && (
                      <Text style={styles.modalSubtitle}>{expandedOutfit.title}</Text>
                    )}
                  </View>
                  ...
                  >
                    <Text
                      style={[
                        ...
                        savedOutfits[expandedOutfit.key] && styles.savedButtonTextSaved,
                      ]}
                    >
      ...
      <View style={styles.outfitRow}>
        {visibleOutfits.map((outfit, index) => (
          <Pressable
            ...
            onPress={() => onOpenOutfit(index + 1, outfit)}
          >
...
const styles = StyleSheet.create({
  ...
  stageCompact: { height: 205 },
  stageExpanded: { height: 365 },
  figureArea: {
    position: "absolute",
    left: "24%",
    right: "24%",
