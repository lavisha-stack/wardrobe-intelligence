...
const BACKEND_URL = process.env.EXPO_PUBLIC_BACKEND_URL || "http://192.168.31.144:8000";
...
function itemImageSource(item) {
  // The original upload is the canonical image for wardrobe/outfit display.
  ...
  return (
    <View style={[styles.stage, expanded ? styles.stageExpanded : styles.stageCompact]}>
      <View style={styles.figureArea}>
        {groups.full && (
          <Image source={itemImageSource(groups.full)} style={styles.fullImage} resizeMode="contain" />
        )}

        {!groups.full && groups.top && (
          <Image source={itemImageSource(groups.top)} style={styles.topImage} resizeMode="contain" />
        )}

        {!groups.full && groups.bottom && (
          <Image source={itemImageSource(groups.bottom)} style={styles.bottomImage} resizeMode="contain" />
        )}

        {groups.shoes && (
          <Image source={itemImageSource(groups.shoes)} style={styles.shoesImage} resizeMode="contain" />
        )}
      </View>
      ...
      {sideItems.map((item, index) => (
        <View
          ...
          style={[
            ...
            index % 2 === 0 ? styles.accessoryLeft : styles.accessoryRight,
            index < 2 ? styles.accessoryTop : styles.accessoryBottom,
          ]}
        >
  ...
  const handleSend = async (overrideMessage) => {
    ...
    try {
      ...
      const response = await fetch(`${BACKEND_URL}/ai-assist`, {
        ...
        body: JSON.stringify({ user_id: user.id, message, conversation_history: conversationHistory }),
      });
          ...
          <>
            {messages.map((message) => (
              <View key={message.id} style={styles.messageBlock}>
                {message.type === "user" && (
                  <View style={styles.userMessage}><Text style={styles.userMessageText}>{message.text}</Text></View>
                )}
                {message.type === "assistant" && (
                  <View style={styles.aiMessage}><Text style={styles.aiLabel}>AI Assist ✨</Text><Text style={styles.aiMessageText}>{message.text}</Text></View>
                )}
                {message.type === "outfits" && <OutfitResults outfits={message.outfits} onOpenOutfit={handleOpenOutfit} />}
              </View>
            ))}
            {loading && (
              <View style={styles.loadingMessage}><Text style={styles.loadingEmoji}>✨</Text><Text style={styles.loadingText}>Styling your wardrobe...</Text></View>
            )}
          </>
        )}
      </ScrollView>
          ...
          {expandedOutfit && (
            <Animated.View
              style={[styles.modalCard, {
                opacity: popAnim,
                transform: [{ scale: popAnim.interpolate({ inputRange: [0, 1], outputRange: [0.88, 1] }) }],
              }]}
            >
              <Pressable onPress={() => {}}>
