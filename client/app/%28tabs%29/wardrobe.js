...
function getFilterCategory(item) {
  const text = [item.category, item.subcategory]
    .filter(Boolean)
    .join(" ")
    .toLowerCase();
  ...
  const deleteItem = async (item) => {
    ...
    try {
      const storagePaths = [
        ...
        getStoragePath(item.normalized_image_url),
      ].filter(Boolean);
  ...
  if (loading) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Text style={styles.statusText}>Loading your wardrobe...</Text>
      </View>
    );
  }
  ...
  if (errorMessage) {
    return (
      <View style={styles.centerContainer}>
        ...
        <Pressable style={styles.retryButton} onPress={fetchWardrobe}>
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View style={styles.emptyContainer}>
            <Text style={styles.emptyTitle}>{items.length === 0 ? "Your wardrobe is empty" : "No items found"}</Text>
            <Text style={styles.emptyText}>
              {items.length === 0 ? "Add your first clothing item to see it here." : "Try a different search or category."}
            </Text>
            {items.length > 0 && (
              <Pressable
        ...
        renderItem={({ item }) => (
          <WardrobeCard item={item} onPress={() => setSelectedItem(item)} />
        )}
      />
  ...
  return (
    <View style={styles.cardWrapper}>
      <Pressable style={styles.card} onPress={onPress}>
        <View style={styles.normalCardFace}>
          <Animated.Image
...
function FocusedCard({ item, onClose, onDelete, deleting }) {
  ...
  const flipToBack = () => {
    ...
    setAnimating(true);
    Animated.timing(rotation, { toValue: 1, duration: 450, useNativeDriver: true }).start(() => {
      setFlipped(true);
      setAnimating(false);
    });
  };
  ...
  const flipToFront = () => {
    if (animating || deleting) return;
    setAnimating(true);
    Animated.timing(rotation, { toValue: 0, duration: 450, useNativeDriver: true }).start(() => {
      setFlipped(false);
      setAnimating(false);
    });
  };
  ...
  const closeOverlay = () => {
    ...
    if (flipped) {
      setAnimating(true);
      Animated.timing(rotation, { toValue: 0, duration: 350, useNativeDriver: true }).start(() => {
        setFlipped(false);
        setAnimating(false);
        animateOut();
      });
    ...
    <View style={styles.overlay}>
      ...
      <Animated.View
        style={[
          ...
          { opacity, transform: [{ perspective: 1200 }, { scale }] },
        ]}
      >
        <View style={styles.focusedCard}>
          <Animated.View
            pointerEvents={flipped ? "none" : "auto"}
            style={[styles.focusedFace, styles.focusedFront, { transform: [{ perspective: 1200 }, { rotateY: frontRotation }] }]}
          >
            <Pressable style={styles.fullFacePressable} onPress={flipToBack} disabled={animating || deleting}>
              ...
              <View style={styles.flipHintContainer}>
                <Text style={styles.flipHint}>Tap to see details</Text>
              </View>
            </Pressable>
          </Animated.View>
          ...
          <Animated.View
            ...
            style={[styles.focusedFace, styles.focusedBack, { transform: [{ perspective: 1200 }, { rotateY: backRotation }] }]}
          >
            <View style={styles.detailsContainer}>
              <View style={styles.backHeader}>
                <View style={styles.backHeaderText}>
                  <Text style={styles.backTitle}>{item.category || "Clothing Item"}</Text>
                  {item.subcategory ? <Text style={styles.subcategory}>{item.subcategory}</Text> : null}
                </View>
                <Pressable style={styles.backButton} onPress={flipToFront} disabled={animating || deleting}>
